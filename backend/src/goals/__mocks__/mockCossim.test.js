...
describe("Test functions that are not getCosSim; does not require mocks", () => {
  ...
  let expectedmakeWordTallyResultstr1 = {
    julie: 1,
    loves: 2,
    me: 2,
    more: 1,
  ...
  let expectedmakeWordTallyResultstr2 = {
    jane: 1,
    likes: 1,
    me: 2,
    more: 1,
  ...
  let expectedmakeAllValuesZeroResultstr1 = {
    julie: 0,
    loves: 0,
    me: 0,
    more: 0,
  ...
  let expectedmakeAllValuesZeroResultstr2 = {
    jane: 0,
    likes: 0,
    me: 0,
    more: 0,
  ...
  test("test function makeWordTally, should expected tally", () => {
    expect(cossimImport.makeWordTally(str1)).toEqual(
      expectedmakeWordTallyResultstr1
    );
    expect(cossimImport.makeWordTally(str2)).toEqual(
      expectedmakeWordTallyResultstr2
    );
  });

  test("test function makeAllValuesZero, should expected vector with all zeros", () => {
    expect(
      cossimImport.makeAllValuesZero(expectedmakeWordTallyResultstr1)
    ).toEqual(expectedmakeAllValuesZeroResultstr1);
    expect(
      cossimImport.makeAllValuesZero(expectedmakeWordTallyResultstr2)
    ).toEqual(expectedmakeAllValuesZeroResultstr2);
  });

  test("test function calccossim, should expected 0.821...", () => {
    expect(cossimImport.calccossim(calccossimParam1, calccossimParam2)).toEqual(
      expectedcalccossimResult
    );
  });
});
...
describe("Test getCosSim; requires mocks", () => {
  test("test function getCosSim; should get expected score", () => {
    ...
    cossimImport.makeWordTally = jest
      .fn()
      .mockImplementation((makeWordTallyResult) => {
    ...
    cossimImport.calccossim = jest.fn().mockImplementation((param1, param2) => {
      if (
        param1 === expectedmakeAllValuesZeroResultstr1 &&
        param2 === expectedmakeAllValuesZeroResultstr2
      ) {
